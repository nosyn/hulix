...
export const getHomeData: () => Promise<any> = async () => {
  ...
  const data = promises.reduce((final, current, index) => {
    final[Object.keys(HomeAPIRoutes)[index]] = current.data.results.map((item: any) => ({
      ...item,
...
export const getWatchMovieContent: (id: string) => Promise<any> = async (id) => {
  ...
  const result = (
    await Promise.all([axios.get(`/movie/${id}`), axios.get(`/movie/${id}/similar`)])
...
export const getSimilarTVs: (id: string) => Promise<any> = async (id) => {
  const data = (await axios.get(`/movie/${id}/similar`)).data.results.map((item: any) => ({
    ...item,
    media_type: 'movie',
  }));
  return data;
...
export const getTVSeasons: (id: string) => Promise<any> = async (id) => {
  ...
  const res = await Promise.all(
    data.seasons.map((item) => axios.get(`/tv/${id}/season/${item.season_number}`))
  );
